...
public class JSUnpacker {
    ...
    public boolean detect() {
        if (packedJS!=null) {
            ...
            Pattern p = Pattern.compile("eval\\(function\\(p,a,c,k,e,[rd]");
    ...
    public String unpack() {
        ...
        try {
            ...
            if(m.find() && m.groupCount() == 4) {
                String payload = Objects.requireNonNull(m.group(1)).replace("\\'", "'");
                String radixStr = m.group(2);
                String countStr = m.group(3);
                String[] symtab = Objects.requireNonNull(m.group(4)).split("\\|");

                int radix = 36;
                int count = 0;
                try {
                    radix = Integer.parseInt(Objects.requireNonNull(radixStr));
                } catch(Exception ignored) {
                }
                try {
                    count = Integer.parseInt(Objects.requireNonNull(countStr));
                } catch(Exception ignored) {
                }
                ...
                while(m.find()) {
                    ...
                    if(value != null && value.length() > 0) {
                        ...
                        replaceOffset += (value.length() - Objects.requireNonNull(word).length());
                    }
                }
